...
In general brokers are used to send and receive messages from queues. APOC has integrated three brokers, RabbitMQ, Simple QueueService, and Kafka. The brokers are
set up in the Neo4j.conf file use two procedures `apoc.broker.send` and `apoc.broker.receive` to send and recieve messages from the brokers.

== Neo4j.conf Setup

Broker APIs use producers (senders) and consumers (receivers) that must be setup for proper functionality. The brokers are configured through the Neo4j.conf file.

Broker configuration always starts with `apoc.broker`. The next part of the configuration is the namespace, followed by the configuration parameters. Each broker being
...
The remaining configuration to set up the broker depends on the type of broker. Currently there are three types of brokers available:

* RabbitMQ (`RABBITMQ`)
* Amazon Web Service's Simple Queue Service (`SQS`)
* Kafka (`KAFKA`)
...
Here is an example of of a configuration to connect to a RabbitMQ running on localhost. Note the namespace of this broker connection is `rabbitmq`.
